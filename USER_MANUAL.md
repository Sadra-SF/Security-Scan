...
### Overview
The Security Scanner includes a comprehensive notification system that supports multiple channels:
- **Email**: SMTP-based notifications with HTML/text templates
- **Slack**: Real-time notifications via Incoming Webhooks
- **Microsoft Teams**: Team channel notifications via webhooks
- **Webhook**: Generic JSON webhooks with HMAC signature support

### Notification Events
- **Scan Completed**: Triggered when a scan finishes successfully
- **Scan Failed**: Triggered when a scan encounters an error
- **Finding Created**: Triggered when new findings are discovered
- **Finding Updated**: Triggered when findings are modified
- **Compliance Violation**: Triggered for compliance rule violations
- **Severity Threshold**: Triggered when findings exceed severity thresholds
...
DEFAULT_FROM_EMAIL=scanner@security-scanner.local
```

#### 2. Create Notification Channels
Use the Django admin or API to create notification channels:
...
Link channels to specific events and severity thresholds:
...
```bash
cd backend/src
python manage.py test_notification --email test@example.com
```

## Demo Walkthrough

### Quick Start Demo
This walkthrough demonstrates the complete security scanning workflow.

#### Prerequisites
1. Backend server running: `cd backend/src && python manage.py runserver`
2. Frontend server running: `cd frontend && npm start`
3. Database migrations applied: `python manage.py migrate`
4. Superuser account created: `python manage.py createsuperuser`
...
3. Fill in project details:
   ...
   - Description: "Security scanning demonstration"
...
3. Configure target settings:
   - Name: "Demo Website"
   - Target URL: `https://httpbin.org` (safe test site)
...
3. Select scan parameters:
   - Target: "Demo Website"
   - Scanner Type: Select "Dynamic Scan" (OWASP ZAP)
   - Scan Profile: Choose appropriate profile

#### Step 5: Run the Scan
1. Click **"Start Scan"**
2. Monitor progress in real-time
3. View findings as they are discovered

#### Step 6: Review Findings
1. Navigate to **Findings** page
2. Filter by severity, category, or target
3. Review detailed information for each finding
4. Update finding status (Open, Resolved, False Positive)
...
1. Configure notification channels as described above
2. Create rules for scan completion events
3. Test notifications with the management command

### Advanced Demo Features

#### Compliance Assessment
1. Navigate to **Compliance** section
2. Select a compliance framework (OWASP Top Ten, PCI DSS, etc.)
3. Run compliance assessment on your target
4. Review compliance gaps and recommendations

#### Scheduled Scans
1. Go to **Schedules** page
2. Create a recurring scan schedule
3. Configure frequency and parameters
4. Monitor automated scan execution
...
**Symptoms**: Scan status shows "Failed" immediately
**Solutions**:
...
4. Check scanner logs: `tail -f backend/src/logs/scanners.log`

#### 2. Database Connection Errors
**Problem**: Unable to connect to PostgreSQL
**Symptoms**: Django server fails to start
**Solutions**:
```bash
...
#### 3. Redis/Celery Connection Issues
**Problem**: Background tasks fail to execute
**Symptoms**: Reports not generating, scans stuck in "Running" status
**Solutions**:
```bash
# Check Redis service
redis-cli ping
...
**Fallback**: The system includes automatic synchronous fallback for notifications when Redis/Celery fails.

#### 4. Email Notifications Not Working
**Problem**: Email notifications fail to send
**Symptoms**: Notification logs show SMTP errors
**Solutions**:
1. Verify SMTP settings in `.env` file
2. Test SMTP connection manually
3. Check email provider settings (Gmail requires app passwords)
4. Use test command: `python manage.py test_notification --email test@example.com`
...
**Solutions**:
- Reduce concurrent scan limits in settings
...
**Symptoms**: Scan status shows "Failed" after long duration
**Solutions**:
- Adjust timeout settings in `SCANNER_DEFAULTS`
- Reduce scan scope or intensity
- Use faster scanner profiles
...
# Notification logs
tail -f backend/src/logs/notifications.log
```
...
- Adjust scanner timeouts based on target size
- Use appropriate scanner profiles for different target types
- Implement scan result caching for repeated targets
- Limit concurrent scans per project
